...
;; constants
(define-constant contract-version "9.0.0")
...
(define-private (assert-admin)
  (match (var-get admin)
    admin-principal (if (is-eq admin-principal tx-sender)
      (ok true)
      err-not-admin
    )
    err-admin-unset
...
(define-private (assert-category (category uint))
  (if (or
      (is-eq category category-coin)
      (is-eq category category-table)
      (is-eq category category-avatar)
    )
    (ok true)
  ...
  (let ((token-id (var-get next-token-id)))
    (begin
      ...
      (map-set drops { id: drop-id }
        (merge drop-data { minted: (+ (get minted drop-data) u1) })
      )
...
(define-private (permit-hash
    (drop-id uint)
    (recipient principal)
    (nonce uint)
  )
  (let ((payload (unwrap-panic (to-consensus-buff? {
...
(define-public (claim-drop (drop-id uint))
  (let ((drop-data (unwrap! (map-get? drops { id: drop-id }) err-drop-not-found)))
    (begin
      ...
      (asserts! (< (get minted drop-data) (get max-supply drop-data))
        err-sold-out
      )
      (asserts!
        (is-none (map-get? claims {
          drop-id: drop-id,
          player: tx-sender,
        }))
        err-already-claimed
...
(define-public (claim-with-permit
    (drop-id uint)
    (nonce uint)
    (signature (buff 65))
  )
    ...
    (begin
      ...
      (let ((signer (var-get claim-signer)))
        (match signer
          pubkey (begin
            (asserts!
              (secp256k1-verify (permit-hash drop-id tx-sender nonce) signature
                pubkey
              )
    ...
    (let ((owner (nft-get-owner? cosmetic token-id)))
      (match owner
        current (begin
          ...
          (unwrap! (nft-transfer? cosmetic token-id sender recipient)
            err-transfer-failed
          )
...
(define-read-only (get-token-uri (token-id uint))
  (let ((token (map-get? tokens { id: token-id })))
    (match token
      record (let ((uri (map-get? drop-uris { drop-id: (get drop-id record) })))
        (match uri
